...
export const BottomNavBar: React.FC<BottomNavBarProps> = ({
  ...
  return (
    ...
    >
      <div className="max-w-xl mx-auto grid grid-cols-5 items-center justify-items-center gap-0.5 sm:gap-1">
        {navItems.map((item) => {
          const Icon = item.icon;
          const isActive = activeTab === item.id || (
            item.id === 'itinerary' && 
            ['transit', 'culinary', 'festivals', 'crafts', 'guide', 'guides'].includes(activeTab)
          );
          return (
            <button
              key={item.id}
